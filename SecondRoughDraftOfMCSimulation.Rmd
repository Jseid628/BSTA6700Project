...
factor_sim <- function(model,n,trt,k_total,t_total,variance, num_timepoints=NULL) {
  ...
  # Commenting this out to use the orthogonal weights.
  ...
  # so that we use the orthogonal weights.
  orthogonal = TRUE
...
for (setting in settings_list) {
  ...
  all_results <- future_map(1:n_sim, function(s) {
    # sketchy putting these inside but apparently that's the way to do it
    library(reticulate)
    library(dplyr)
    library(osqp)
...
sim_time <- readRDS("results/sim_timeOrtho_4-26.rds")
sim_time
