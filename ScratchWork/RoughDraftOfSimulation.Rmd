...
factor_sim <- function(model,n,trt,k_total,t_total,variance, num_timepoints=NULL) {
  ...
  morph <- reticulate::import_from_path("morphData", path = ".")
  lq    <- reticulate::import_from_path("learnQ", path = ".")
  ...
  imbalance_avg <- rmse(t(out_control_avg),out_trt_avg,w_avg)
    ...
    return(list("oracle_bias" = c(oracle_bias_sep, oracle_bias_cat, oracle_bias_avg,as.numeric(oracle_bias_Q)), 
...
for (setting in settings_list) {
  ...
  all_results <- future_map(1:n_sim, function(s) {
    ...
    set.seed(s)
    factor_sim(model, n, trt, k_total, t_total, variance = 1, num_timepoints = 40)
    
  }, .options = furrr_options(seed=NULL,globals = c("factor_sim", "model", "n", "trt",
                                           "k_total", "t_total", "synth_qp",
                                           "SCMbias", "rmse")))
  ...
  saveRDS(all_bias_sim_long, file = "results/sim_results.rds")
}

#### REMOVING THIS LOGIC TO SEE IF I CAN REPRODUCE IT PROPERLY #####
...
cat("Elapsed time:", elapsed["elapsed"], "seconds\n")
```

```{r}
all_bias_sim_long <- readRDS("results/sim_results.rds")
